...
const translations = {
    es: {
        app_title: 'Accelevation Control',
        tab_loans: '📋 Préstamos',
        ...
        placeholder_category: 'Ej: Eléctrica, Manual...',
    ...
    en: {
        ...
        placeholder_category: 'Ex: Electric, Manual...',
...
function abrirModalHerramienta(herramienta = null) {
    ...
    document.getElementById('herramientaCategoria').value = herramienta ? (herramienta.categoria || '') : '';
...
async function guardarHerramienta(e) {
    ...
    const data = {
        ...
        categoria: document.getElementById('herramientaCategoria').value.trim() || t('category_none'),
...
async function renderHerramientas() {
    ...
    const grouped = herramientas.reduce((acc, h) => {
        const cat = h.categoria || t('category_none');
        if (!acc[cat]) acc[cat] = [];
        acc[cat].push(h);
        return acc;
    }, {});

    // Sort categories (Sin Categoría last, then alpha)
    const sortedCats = Object.keys(grouped).sort((a, b) => {
        if (a === t('category_none')) return 1;
        if (b === t('category_none')) return -1;
        return a.localeCompare(b);
    });

    container.innerHTML = sortedCats.map(cat => {
        const isCollapsed = collapsedCategories.has(cat);
        const itemsHtml = grouped[cat].map(hItem => {
        ...
        return `
            <div class="category-section ${isCollapsed ? 'collapsed' : ''}">
                <h3 class="category-header" onclick="toggleCategory('${cat.replace(/'/g, "\\'")}')">
                    <span>${cat}</span>
